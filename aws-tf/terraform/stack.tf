...
resource "aws_instance" "linux_box" {
  ...
  user_data_base64 = base64encode(templatefile("./bootstrapping/amazon-linux-private-repo.txt", { github_pat = var.github_pat, github_user_name = var.github_user_name, github_repo_name = var.github_repo_name }))
  key_name         = "ssh-windows-to-linux"
